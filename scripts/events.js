...
function prepareDistanceData(data) {
  ...
  for (var i = 0; i < 53; i++) {
    ...
    let endDate = am4core.time.round(new Date(date), "week", 1);
  ...
  am4core.array.each(data, function(di) {
    ...
    let weekNumber = am4core.utils.getWeek(date);

    if (weekNumber == 1 && date.getMonth() == 11) {
      weekNumber = 53;
    }

    let distance = am4core.math.round(di["Distance"] / 1000, 1);
...
// Create series
let columnSeries = chart.series.push(new am4charts.RadarColumnSeries());
...
// weekday axis
let weekDayAxis = chart.yAxes.push(new am4charts.CategoryAxis());
...
// bubble series
let bubbleSeries = chart.series.push(new am4charts.RadarSeries())
...
let link = chart.createChild(am4core.TextLink);
